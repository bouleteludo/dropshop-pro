...
export async function POST(req: NextRequest) {
  ...
  const subtotal = lineItems.reduce((sum, item) => sum + (item.price_data.unit_amount / 100) * item.quantity, 0);
  ...
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      shipping_options: [
        {
          shipping_rate_data: {
            ...
            display_name: shipping === 0 ? "Livraison offerte" : "Livraison standard",
            delivery_estimate: {
      ...
      metadata: {
        ...
        shipping: String(Math.round(shipping * 100)),
      },
    });
